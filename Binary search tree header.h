...
node<T>* bin_tree_main<T>::find_succeesor(node<T>* current, T value)
{
	...
	node<T>* value_node;
	value_node = binary_search(current,value);
...
node<T>* bin_tree_main<T>::least_common_ancestor(T value1, T value2)
{
	...
	while (current1->data != current2->data&&current1->parent&&current2->parent)
	{
...
void bin_tree_exec::command_control(bin_tree_main<T> bin_tree)
{
	...
	do
	{
		...
		case 16:	
			...
			cin >> value1 >> value2;
			cout << " is " << bin_tree.least_common_ancestor(value1, value2)->data << endl;
			break;
		case 18:
			forest(bin_tree);
			break;
